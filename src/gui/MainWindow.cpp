...
void MainWindow::setMainWindowTitle() {
  setWindowTitle(QString(tr("Parsec Wallet %1")).arg(Settings::instance().getVersion()));
}
...
void MainWindow::createLanguageMenu(void)
{
  ...
  m_langPath = "/usr/local/share/parsec/parsecwallet/languages";
#else
  m_langPath = "/opt/parsec/languages";
#endif
  QDir dir(m_langPath);
...
//  QMessageBox::information(nullptr, QObject::tr("Help"), cmdLineParser.getHelpText());
    QMessageBox *msg = new QMessageBox(QMessageBox::Information, QObject::tr("Help"),
                       ...
                       QMessageBox::Ok, this);
    msg->setInformativeText(tr("More info can be found at www.parsecnode.com in Documentation section"));
...
void MainWindow::walletOpened(bool _error, const QString& _error_text) {
  if (!_error) {
    ...
    setWindowTitle(QString(tr("%1 - Parsec Wallet %2")).arg(Settings::instance().getWalletFile()).arg(Settings::instance().getVersion()));
...
void MainWindow::createTrayIcon()
{
    ...
    QString toolTip = QString(tr("Parsec Wallet %1")).arg(Settings::instance().getVersion());
