...
void SendFrame::parsePaymentRequest(QString _request) {
  ...
  if(_request.startsWith("parsec://", Qt::CaseInsensitive))
  {
    _request.replace(0, 13, "parsec:");
  }
  if(!_request.startsWith("parsec:", Qt::CaseInsensitive)) {
    QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with parsec:"), QtCriticalMsg));
    return;
  }
...
void SendFrame::sendClicked() {
  ...
  if (m_ui->donateCheckBox->isChecked()) {
    CryptoNote::WalletLegacyTransfer walletTransfer;
    walletTransfer.address = "PARSkAmcBG8ZWqgfCu65fddB1yxzbTpGF6wzyLBsTUmoJUd1WmB8Bgj9h9LGbmVQEEEYAFHVJCWCVBEeEfsbK5ay6tSDmDup7i";
