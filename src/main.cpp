...
int main(int argc, char* argv[]) {
  ...
  QSettings parsecKey("HKEY_CLASSES_ROOT\\parsec", QSettings::NativeFormat);
  parsecKey.setValue(".", "Parsec Wallet");
  parsecKey.setValue("URL Protocol", "");
  QSettings parsecOpenKey("HKEY_CLASSES_ROOT\\parsec\\shell\\open\\command", QSettings::NativeFormat);
  parsecOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = Parsec URL Handler\\nGenericName = Parsec\\nComment = Handle URL Sheme parsec://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/parsec;\\nIcon = Parsec-Wallet' | tee /usr/share/applications/parsec-handler.desktop";
